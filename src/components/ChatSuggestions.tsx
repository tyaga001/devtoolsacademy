    ...
    return (
        <div className='w-full flex py-6 flex-col gap-6 items-center'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 w-full gap-5'>
                <motion.div onClick={() => { handleSend(`Make me understand this blog : ${title}?`) }} initial={{ y: 20, opacity: 0 }} whileHover={{ scale: 1.05 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.1 }} exit={{ y: 20, opacity: 0 }} className='px-4 py-5 border border-white/40 rounded-[14px] bg-black/70 text-sm font-[200] text-white/60 w-full hover:bg-black hover:text-white hover:border-white/90 duration-200 cursor-pointer'>
                    Make me understand this blog : {title}?
                </motion.div>
                <motion.div onClick={() => { handleSend(`What does "${description}" mean in this blog? `) }} initial={{ y: 20, opacity: 0 }} whileHover={{ scale: 1.05 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.1 }} exit={{ y: 20, opacity: 0 }} className='px-4 py-5 border border-white/40 rounded-[14px] bg-black/70 text-sm font-[200] text-white/60 w-full hover:bg-black hover:text-white hover:border-white/90 duration-200 cursor-pointer'>

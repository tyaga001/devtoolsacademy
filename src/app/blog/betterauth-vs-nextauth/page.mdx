...
export const metadata = getMetadata({
  ...
  title: "BetterAuth vs NextAuth: Which Authentication Library Should You Choose?",
  description: "Compare BetterAuth and NextAuth across authentication methods, plugin support, extensibility, scalability, and best practices to decide which fits your Next.js app best.",
})
...
<Image src={image6} alt="auth" width={1200} height={600} />
...
Both libraries implement sessions with security in mind: cookies are HTTP-only and CSRF-protected. For Better Auth, cookies are only cookies are secure only when the server is running in production mode. You can force cookies to be always secure by setting useSecureCookies to true in the advanced object in the auth options.

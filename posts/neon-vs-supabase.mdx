...
|                                                                                                                   |
| ----------------------------------------------------------------------------------------------------------------- |
| NOTICE:  Welcome to Neon! Authenticate by visiting:     https\://console.neon.tech/psql\_session/6d32af5ef8215b62 |
...
|                                                                                                                                                                     |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POSTGRES\_URL="postgres\://postgres.cfcxynqnhdybqtbhjemm:\[YOUR-PASSWORD]@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres?workaround=supabase-pooler.vercel" |
...
Supabase is built on open-source tools designed to be scalable and easy to use. Supabase is not a 1-to-1 mapping of Firebase. While Supabase aims to offer many of the features that Firebase provides, its approach is different, focusing on open-source technologies and supporting existing tools whenever possible.
...
|              **CSV Import**              |    Use the `\copy` command in `psql` to import CSV data directly into tables. Ideal for structured, smaller datasets.   |                   Supabase Dashboard supports CSV import up to 100MB through a user-friendly interface.                  |
|       **pg\_dump and pg\_restore**       | Standard method for migrating data from another Postgres database using `pg_dump` to export and `pg_restore` to import. |        pgloader can be used for large-scale imports from various databases, offering robust configuration options.       |
|               **CLI Tools**              |     `@neondatabase/pg-import` CLI for simpler migrations, though it is experimental and best for smaller databases.     |    Supabase API allows programmatic data import, useful for automation and providing control over the import process.    |
|     **Project-to-Project Migration**     | Directly pipe data from one Neon project to another using `pg_dump` and ****`pg_restore`, efficient for small datasets. |      Not explicitly mentioned, but possible via pgloader or the COPY command for complex or larger data migrations.      |
| **AWS Database Migration Service (DMS)** |        Suitable for complex migrations from various database engines like MySQL, Oracle, and SQL Server to Neon.        | Supabase supports migration from Amazon RDS (Postgres, MySQL, MS SQL) using standard tools or dedicated migration tools. |
|           **Heroku Migration**           |      Use Heroku CLI’s `pg:pull` command to migrate data from Heroku Postgres to Neon, convenient for Heroku users.      |        Migrate from Heroku Postgres using **pg\_dump** and psql CLI tools or a Heroku to Supabase migration tool.        |
|             **COPY Command**             |     Use the `COPY` command for efficient bulk data loading, ideal for large data loads and production environments.     |              Similarly, the COPY command in Postgres is highly efficient for bulk data loading in Supabase.              |
|            **MySQL Migration**           |           Use pgloader to migrate MySQL databases to Neon, transforming data to a Postgres-compatible format.           |       Use pgloader or a migration tool in Google Colab to migrate MySQL databases to Supabase's Postgres database.       |
...
Both platforms prioritize secure connections using SSL/TLS encryption. Supabase enhances security with JWT and RLS, while Neon employs the strict `verify-full` SSL mode, ensuring that a trusted CA issues the server’s certificate and that the server’s hostname matches the certificate. Additionally, Neon enforces high-entropy passwords for all PostgreSQL roles to prevent brute-force attacks.

Supabase uses the Vault feature to manage secrets, providing encrypted storage for sensitive information such as environment variables and API keys. On the other hand, Neon encrypts inactive data using the XTS-AES-256 block cipher on NVMe SSD volumes, ensuring that data remains secure and unreadable without the proper encryption key.

Authentication Supabase provides authentication through JWT, integrated with RLS for user-specific data access. Neon goes a step further by integrating with multiple authentication platforms, including Auth0, Clerk, and Okta, offering advanced features like social login, multi-factor authentication, and secure user data management.

 In terms of compliance, Neon has achieved SOC 2 Type 1 and Type 2 compliance, demonstrating adherence to high security and privacy standards. Supabase follows PostgreSQL’s robust security features but does not specifically mention SOC 2 compliance.

|                             |                                        |                                                                    |
| :-------------------------: | :------------------------------------: | :----------------------------------------------------------------: |
|         **Feature**         |              **Supabase**              |                              **Neon**                              |
|    **Secure connections**   |    SSL/TLS encryption, JWT, and RLS    | SSL/TLS encryption with `verify-full` mode, high-entropy passwords |
|      **Access control**     | RLS, Column Level Privileges, Policies |                  IP allowlists, protected branches                 |
| **Data-at-Rest encryption** |       Vault for managing secrets       |             XTS-AES-256 encryption on NVMe SSD volumes             |
|        **Compliance**       |  Follows PostgreSQL security features  |                 SOC 2 Type 1 and Type 2 compliance                 |
|      **Authentication**     |               JWT and RLS              |               Integration with Auth0, Clerk, and Okta              |
|    **Security reporting**   |      No specific process mentioned     |     Follows securitytxt.org standard for transparent reporting     |
...
**Autosuspend** The Autosuspend feature allows Neon compute instances to scale to zero after a period of inactivity, with the default set at 300 seconds (5 minutes). This feature is customizable for paid users and helps reduce compute usage and costs by suspending inactive computes. Autosuspend can be adjusted to avoid or reduce the frequency of cold starts.
...
**IP Allow and Protected Branches** The IP Allow feature restricts database access to trusted IP addresses, enhancing data security. Users can configure an allowlist and apply IP restrictions to specific branches by designating them as protected. This feature is available with the Neon Scale plan.
...
**Branching** Supabase supports database branching for testing and previewing changes, similar to version control systems for code such as Git.

**Terraform provider** Supabase can manage infrastructure via Terraform, allowing Infrastructure as Code (IaC) management.

**Read replicas** Supabase supports deploying read-only databases across multiple regions for lower latency and better resource management.

**Realtime features** Supabase provides real-time features like receiving database changes through websockets, broadcasting messages between users, and synchronizing shared state across users.

**Authentication features** Supabase supports various authentication methods, including email login, social login, phone login, passwordless login, and row-level security. It also provides Captcha protection and server-side authentication helpers for popular frameworks.

**Storage features** Supabase offers file storage, content delivery network (CDN) for caching files, smart CDN for automatic revalidation, image transformations, resumable uploads, and S3 compatibility.

**Edge functions** Supabase supports globally distributed TypeScript functions via Deno, regional invocations, and NPM compatibility for executing custom business logic.

**Project management** Supabase provides a CLI for local development and deployment, a management API for programmatic project management, and official client libraries for various languages.

|                             |                                                                                                                                                                                                                                                                 |                                                                                                                                                           |
| :-------------------------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------: |
|         **Feature**         |                                                                                                                             **Neon**                                                                                                                            |                                                                        **Supabase**                                                                       |
|       **Autoscaling**       |                                                                                              Yes, dynamically adjusts compute resources with Kubernetes and NeonVM                                                                                              |                                                   Yes, adjusts compute resources based on real-time load                                                  |
|       **Autosuspend**       |                                                                                                      Yes, scales compute instances to zero after inactivity                                                                                                     |                                                                             No                                                                            |
|        **Branching**        |                                                                                             Yes, with extensive support for development, testing, and data recovery                                                                                             |                   Supabase supports database branching for testing and previewing changes, similar to version control systems for code.                   |
|       **Time travel**       |                                                                                                          Yes, for analyzing database changes over time                                                                                                          |                                                                             No                                                                            |
|       **Schema Diff**       |                                                                                                       Yes, for comparing database schemas between branches                                                                                                      |                                                                             No                                                                            |
|   **Logical replication**   |                                                                                                               Yes, for CDC and real-time analytics                                                                                                              |                                                                             No                                                                            |
|         **IP Allow**        |                                                                                                       Yes, for trusted IP addresses and protected branches                                                                                                      |                                                                  Yes, restrict IP ranges                                                                  |
|    **Protected branches**   |                                                                                                         Yes, with IP restrictions for specific branches                                                                                                         |                                                                             No                                                                            |
|    **Postgres database**    |                                                                                                                      Full Postgres database                                                                                                                     |                                                                   Full Postgres database                                                                  |
|  **Secrets and encryption** |                                                                                                                               Yes                                                                                                                               |                                                                  Yes, via Supabase Vault                                                                  |
|     **Database backups**    |                                                                                                                 Yes, with Point-in-Time recovery                                                                                                                |                                                       Yes, daily backups with Point in Time recovery                                                      |
|   **Network restrictions**  |                                                                                                              Yes, IP Allow for trusted IP addresses                                                                                                             |                                                                  Yes, restrict IP ranges                                                                  |
|     **SSL enforcement**     |                                                                                                                               Yes                                                                                                                               |                                                                            Yes                                                                            |
|      **Read replicas**      |                                                                                                    Yes, in the same region, with autoscaling and autosuspend                                                                                                    |                                                                Yes, across multiple regions                                                               |
| **Auto-generated REST API** |                                                                                                                        Yes, via Neon API                                                                                                                        |                                                                     Yes, via PostgREST                                                                    |
|    **Project Management**   |                                                                                                                 Yes, with CLI and management API                                                                                                                |                                         Yes, with CLI, management API, and client libraries for multiple languages                                        |
|     **Vector database**     | Neon supports the pgvector open-source extension, which enables Postgres as a vector database for storing and querying vector embeddings. By enabling Postgres as a vector database, you can keep your data in the open source database that you know and trust |                                                                            Yes                                                                            |
|    **Database webhooks**    |                                                                                                                                No                                                                                                                               |                                                                            Yes                                                                            |
|    **Realtime Features**    |                                                                                                                                No                                                                                                                               |                                                  Yes, including Postgres changes, broadcast, and presence                                                 |
| **Authentication Features** |                                                                                                               No specific authentication features                                                                                                               | Yes, including email login, social login, phone login, passwordless login, row-level security, Captcha protection, and server-side authentication helpers |
|         **Storage**         |                                                                  Neon uses a multi-tenant key-value store for Postgres pages, consisting of Safekeepers, Pageservers, and cloud object storage.                                                                 |                        Yes, including file storage, CDN, smart CDN, image transformations, resumable uploads, and S3 compatibility                        |
|      **Edge Functions**     |                                                                                                                                No                                                                                                                               |                                               Yes, via Deno with regional invocations and NPM compatibility                                               |
...
Supabase allows you to set up configurable IP range restrictions for database connections, which are managed via the dashboard or CLI, enhancing security. These restrictions apply to both IPv4 and IPv6, though they do not cover HTTPS APIs like PostgREST, Storage, and Auth.
...
Additionally, Neon supports third-party tools like PgHero and PgAdmin for advanced performance analytics, and the **pg\_stat\_statements** extension helps optimize query performance.
...
|                                                                                                                     |
| ------------------------------------------------------------------------------------------------------------------- |
| npx @neondatabase/pg-import --source \<source\_database\_connection\_string> --backup-file-path \<dump\_file\_name> |

In contrast, Supabase provides automated daily backups and PITR, with daily backups accessible for 7 days on Pro plans and 30 days on Enterprise plans. For databases over 40GB, physical backups are used, ensuring performance with minimal disruption. PITR, available as an add-on, allows recovery to any point in time with high granularity, suitable for projects requiring stringent Recovery Point Objectives (RPO). Supabase uses pg\_dumpall for daily backups and WAL-G for PITR, ensuring data integrity and efficient disaster recovery


### Support<a id="support"></a>

Neon offers four support levels: **Community, Standard, Priority, and Enterprise**. Community support is accessible through the Neon Discord Server, included with the Free Tier. Standard support, available with the Launch plan, adds support tickets with a two-business-day response time. Priority support, part of the Scale plan, includes prioritized tickets and video chats. Enterprise support, aligned with the Enterprise plan, includes a dedicated Customer Success Team and SLAs.
...
Supabase also has tiered support: Community support is included with the Free plan, while email support is available in the Pro plan. The Team plan offers priority email support, SLAs, and daily backups for 14 days. The Enterprise plan provides premium support, a designated support manager, a private Slack channel, and custom SLAs. 

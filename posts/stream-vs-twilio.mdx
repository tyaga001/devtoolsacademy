...
When it comes to communication use cases, software developers mostly think of WebRTC or WebSockets, but if you want to use some additional features in communication then most of the software folks go for Twilio. Twilio is quite popular in the industry, but today let’s dive deep into the solution as well as Stream by creating an automated customer support bot.
...
Stream is mostly used for in-app chat use cases, use cases like social apps or e-commerce, and gaming applications. Twilio is dope when it comes to a multi-modal communication API provider
...
In the next section, we'll explore how to develop a customer support bot using these two services 👀


## Let’s cook Twillio customer support bot 🧑‍🍳
